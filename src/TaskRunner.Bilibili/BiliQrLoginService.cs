...
public sealed record BiliQrPollResult(
...
public sealed class BiliQrLoginService(
    HttpClient http,
    IBilibiliCookieStore cookieStore,
    ILogger<BiliQrLoginService> logger) : IBiliQrLoginService
{
    public async Task<BiliQrGenerateResult> GenerateAsync(CancellationToken cancellationToken)
    {
        ...
        var payload = await response.Content.ReadFromJsonAsync<BiliApiResponse<QrCodeData>>(BiliJson.Options, cancellationToken)
        ...
        var url = payload.Data.Url
            ?? throw new InvalidOperationException("生成二维码失败：缺少 url。");
        return new BiliQrGenerateResult(payload.Data.QrcodeKey, url, ToDataUrl(url));
    }

    public async Task<BiliQrPollResult> PollAsync(
        string qrcodeKey,
        bool saveOnSuccess,
        CancellationToken cancellationToken)
    {
        ...
        var url =
            $"https://passport.bilibili.com/x/passport-login/web/qrcode/poll?qrcode_key={Uri.EscapeDataString(qrcodeKey.Trim())}&source=main_mini";
        using var request = new HttpRequestMessage(HttpMethod.Get, url);
        request.Headers.TryAddWithoutValidation("Referer", "https://www.bilibili.com/");
        using var response = await http.SendAsync(request, cancellationToken);
        if (!response.IsSuccessStatusCode)
        {
            return new BiliQrPollResult(BiliQrLoginStatus.Error, $"HTTP {(int)response.StatusCode}", null, false);
        }
        ...
        {
            return new BiliQrPollResult(
                BiliQrLoginStatus.Error,
                payload?.DisplayMessage ?? "检测接口异常",
                null,
                false);
        }
        ...
        return payload.Data.Code switch
        {
            ...
            _ => new BiliQrPollResult(
                BiliQrLoginStatus.Waiting,
                payload.Data.Message ?? $"code={payload.Data.Code}",
                null,
                false)
        };
    }

    private async Task<BiliQrPollResult> OnSuccessAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!parsed.HasLoginTokens)
        {
            return new BiliQrPollResult(
                BiliQrLoginStatus.Error,
                "扫码成功但 Cookie 缺少 SESSDATA/bili_jct",
                cookie,
                false);
        }
