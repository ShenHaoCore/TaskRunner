...
public sealed class AdminDashboardAuthorizationFilter : IDashboardAuthorizationFilter
{
    public bool Authorize(DashboardContext context)
    {
        ...
        if (http.Request.Headers.TryGetValue(AdminAuthDefaults.HeaderName, out var header)
            && string.Equals(header.ToString(), options.AdminApiKey, StringComparison.Ordinal))
        {
            return true;
        }

        if (http.Request.Cookies.TryGetValue(AdminAuthDefaults.CookieName, out var cookie)
            && string.Equals(cookie, options.AdminApiKey, StringComparison.Ordinal))
        {
            return true;
        }
            ...
            && authorization.StartsWith("Basic ", StringComparison.OrdinalIgnoreCase))
        {
            try
            {
                ...
                if (parts.Length == 2 && string.Equals(parts[1], options.AdminApiKey, StringComparison.Ordinal))
                {
                    return true;
                }
            }
